...
memorySvc *memory.Service,
hitlStore *hitl.Store,
ollamaURL, planModel, evalModel string,
) *ChatHandler {
return &ChatHandler{
pool:       pool,
checkpoint: cp,
...
orch := h.buildOrchestrator(ctx, stub, events, spec, runReq.SessionID, true)
...
h.maybeFlushMemory(ctx, req.UserID, req.SessionID, result, spec)
...
events := make(chan dag.SSEEvent, 64)
stub := langgraphv1.NewAgentServiceClient(h.pool.Next())
orch := h.buildOrchestrator(ctx, stub, events, spec, runReq.SessionID, false)
go func() {
for range events {
}
}()
...
ollama := planner.NewOllamaClient(h.ollamaURL)
...
func (h *ChatHandler) maybeFlushMemory(ctx context.Context, userID, sessionID string, result *dag.RunResult, spec *agentstore.AgentSpec) {
if h.memorySvc == nil {
return
}
